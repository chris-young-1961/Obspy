...
@author: chrisyoung
"""
# Plots map + record section for 2017 NK Nuke
import obspy
...
freqmin = '0.8'                 # BP filter min frequency
freqmax = '3.0'                 # BP filter max frequency
min_dist_deg = 00.
max_dist_deg = 40.  
y_lim = [min_dist_deg,max_dist_deg]         #max station distance
time_duration = 20 * 60.     #time duration in seconds to plot after event time
lead_time = 5 * 60.             #lead time in seconds before event
scale_fac = (y_lim[1] - y_lim[0])/10       #wfm amplitude scaling
font_size = 5                              #sta label fontsize
##set parameters for map
small_circle_radii_deg = [10,20,30,40]          #radii of small circles around event
...
exclude_stations = ['CHTO','DAV','BILL']
# exclude_stations = ['DHK1','CY302','CY303','CY304','CY401','CY601','CY602',
#                     'CY603','CY604','CY605','CY606','CRES','BEL','GURO',
#                     'KIR1','RAYN','BSR2','LIT']
...
inv = c_iris.get_stations(network = network, station = '*', location = location,
...
st = c_iris.get_waveforms(network=network, station = sta_str, location = location,
...
for tr in st_filter:
    if tr.stats.station in exclude_stations:
        ...
        # time = np.arange(0,tr.stats.endtime-tr.stats.starttime+tr.stats.delta,tr.stats.delta)
...
# Add map, left side
ax2 = fig.add_subplot(1, 2, 1, projection=ccrs.Orthographic(central_longitude=origin.longitude, 
...
plt.figure(fig)
plt.savefig('EventRecSectionPlusMap.png', dpi = 300)